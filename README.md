...
OCX mirror for [Task](https://github.com/go-task/task). One repository, one spec
directory per package.

| Package | Spec | Publishes to | Announced as | Upstream SPDX |
|---|---|---|---|---|
| [go-task/task](https://github.com/go-task/task) | [`task/mirror.yml`](task/mirror.yml) | `ghcr.io/ocx-contrib/go-task/task` | `ocx.sh/go-task/task` | `MIT` |
...
> `ocx.sh/go-task`. `go-task/task` is the grouped successor: the namespace takes
> upstream's org and the package segment takes the publish-name — upstream's
> repo is `go-task/task` and the binary it ships is `task`.
...
`task` publishes six platform entries: both Linux arches, both macOS arches and
both Windows arches. Upstream builds Task as a pure-Go binary without cgo, so
...
the `assets:` block in `task/mirror.yml`.

Upstream also ships `linux/{386,arm,riscv64}`, `freebsd/*` and `windows/386`
tarballs plus `.deb`/`.rpm`/`.apk` installers — none has a GitHub Actions
runner or is bundle-shaped, so the anchored asset regexes exclude them.

## Editing

| File | Edit | Regenerate after |
|------|------|------------------|
| `mirror-base.yml`, `task/mirror.yml` | hand | yes — see below |
| `task/{metadata.json,CATALOG.md,logo.*}` | hand | — |
| `task/tests/smoke.star` | hand | — |
| `.github/workflows/*.yml` | **generated — never hand-edit** | re-run when a spec changes |

```bash
ocx-mirror package pipeline generate ci --spec task/mirror.yml
```
...
Task ships as a tarball holding `task` **flat at its root** beside `LICENSE`,
`README.md` and a `completion/` directory, so `strip_components: 0` lands the
executable at the content root and the bundle's only PATH entry is a bare
`${installPath}`. `bin_scan` only looks *below* an `${installPath}/<dir>`
entry, so `auto`/`verify` is rejected at spec load with exit 65 — the scan
would inspect no file and pass green whatever the archive contained.
`mirror-base.yml` therefore sets `bin_scan: off` and `task/metadata.json`
hand-lists `binaries: ["task"]`. The `completion/` directory is data, not an
interface path, and is deliberately not on PATH.
